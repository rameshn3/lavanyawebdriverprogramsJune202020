...
public class SelectRandomOptionTest {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		//Create objetc for Random class
		Random randObj=new Random();
		
		for(int i=1;i<=5;i++) {
			//generate random number
			int ranNumb=randObj.nextInt(opts.size());
			//select an option by position
			sel.selectByIndex(ranNumb);
			
			Thread.sleep(1000);
			System.out.println("selected option "+ranNumb+ " is :"+sel.getFirstSelectedOption().getText());
		}
		Thread.sleep(3000);
		//close the browser
		driver.close();
