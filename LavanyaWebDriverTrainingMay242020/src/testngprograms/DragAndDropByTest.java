...
public class DragAndDropByTest {
  ...
  public void dragAndDropByTest() throws InterruptedException {
	  driver.get("https://jqueryui.com/draggable/");
		wait.until(ExpectedConditions.titleContains("Draggable | jQuery UI"));
		Assert.assertEquals("Draggable | jQuery UI", driver.getTitle());
		...
		//act.dragAndDropBy(src, 90,120).perform();
		act.clickAndHold(src).perform();
		act.moveByOffset(100, 150).perform();
		act.release().perform();
		Thread.sleep(3000);
